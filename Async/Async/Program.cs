...
namespace Examples.AdvancedProgramming.AsynchronousOperations
{
    ...
    public class DemonstrateAsyncPattern
    {
        ...
        public void FactorizeUsingTask()
        {
            var background = Task.Factory.StartNew(
                () =>
                {
                    int primeFactor1 = 0;
                    int primeFactor2 = 0;
                    bool result = PrimeFactorFinder.Factorize(1000589023, ref primeFactor1, ref primeFactor2);
                    return new
                    {
                        Result = result,
            ...
            background.ContinueWith(task =>
                {
                    Console.WriteLine("{0}/{1}  [Succeeded {2}]",
                    task.Result.Factor1,
                    task.Result.Factor2,
                    task.Result.Result);
        ...
        public async Task<Result> FactorizeAsync(int n)
        {
            int primeFactor1 = 0;
            int primeFactor2 = 0;
            bool result = PrimeFactorFinder.Factorize(n, ref primeFactor1, ref primeFactor2);
            await Task.Delay(500);
            return new Result { result = result, factor1 = primeFactor1, factor2 = primeFactor2 };
        }

        public async void FactorizeUsingAsync()
        {
            Result r = await FactorizeAsync(1000589023);
            Console.WriteLine("** {0}/{1}  [Succeeded {2}]",
            r.factor1,
            r.factor2,      
            r.result);
